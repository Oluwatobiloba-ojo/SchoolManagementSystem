...
public class AppQuizService implements QuizService{
    ...
    public List<QuizResponse> findAllQuizBelonging(Teacher teacher) {
        ...
        return quizzes.stream()
                .map(quiz -> new QuizResponse(quiz.getTitle(), questionService.findAllQuestionByQuiz(quiz))).toList();
    }

    @Override
    public QuizResponse createQuiz(Teacher teacher, UploadQuizRequest request) throws IOException {
        Quiz quiz = new Quiz();
        quiz.setTitle(request.getTitle());
        quiz.setWriter(teacher);
        quiz.setDescription(request.getDescription());
        List<QuestionResponse> questionResponses = questionService.createQuestions(request.getFile(), quiz);
